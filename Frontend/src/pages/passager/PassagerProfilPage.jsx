...
const NAV = [
    ...
    {
        key: 'historique', label: 'Historique des trajets', desc: 'Vos réservations passées',
...
export default function PassagerProfilPage() {
    const { user } = useAuth()
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="space-y-0">
                <FieldRow label="Prénom" editing={editing} value={user?.prenom}
                    input={<Input value={form.prenom} onChange={v => setForm(f => ({ ...f, prenom: v }))} />} />
                <FieldRow label="Nom" editing={editing} value={user?.nom}
            ...
            <div className="flex items-center gap-3 pt-2">
                ...
                {saved && (
                    <span className="text-sm text-brand-600 font-semibold flex items-center gap-1.5">
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="6" stroke="currentColor" strokeWidth="1.3" /><path d="M4.5 7l2 2 3-3.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
...
function HistoriqueSection() {
    ...
    useEffect(() => {
        getMesReservations()
            .then(({ data }) => setReservations(data.filter(r =>
    ...
    const statusCfg = {
        ACCEPTEE:              { label: 'Terminé',              cls: 'bg-brand-50 text-brand-700 border-brand-200' },
    ...
    const getEffectiveStatus = (r) =>
        r.statut === 'ACCEPTEE' && r.annonce?.statut === 'TERMINEE' ? 'ACCEPTEE' : r.statut

    const filtered = filter === 'TOUS' ? reservations : reservations.filter(r => getEffectiveStatus(r) === filter)
    ...
    if (reservations.length === 0) return <p className="text-sm text-[#8AA899]">Aucun trajet dans l'historique.</p>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                ...
                {[
                    ...
                    { key: 'ACCEPTEE',            label: 'Terminés',          cls: statusCfg.ACCEPTEE.cls },
                    { key: 'REFUSEE_CONDUCTEUR',  label: 'Refusées',          cls: statusCfg.REFUSEE_CONDUCTEUR.cls },
                    { key: 'ANNULEE_PASSAGER',    label: 'Annulées par vous', cls: statusCfg.ANNULEE_PASSAGER.cls },
                    { key: 'ANNULEE_CONDUCTEUR',  label: 'Annulées (cond.)',  cls: statusCfg.ANNULEE_CONDUCTEUR.cls },
                ].map(({ key, label, cls }) => (
            ...
            {paged.length === 0 ? (
                ...
                <div className="border border-[#EBEBEB] rounded-2xl overflow-hidden divide-y divide-[#EBEBEB]">
                    {paged.map(r => {
                        const cfg = statusCfg[getEffectiveStatus(r)] ?? statusCfg.ANNULEE_CONDUCTEUR
                        const depart = r.annonce?.trajet?.villeDepart ?? '—'
                        const arrivee = r.annonce?.trajet?.villeArrivee ?? '—'
                        const total = r.annonce?.prixParPlace && r.nombrePlaces
                            ? `${(r.annonce.prixParPlace * r.nombrePlaces).toFixed(0)} MAD`
                            : '—'
                        return (
                            <div key={r.id} className="px-5 py-4 bg-white hover:bg-[#FAFAF9] transition-[background-color] duration-150">
                            ...
                            {Array.from({ length: 5 }, (_, i) => (
                                <svg key={i} width="14" height="14" viewBox="0 0 14 14" fill="none">
                                    <path d="M7 1.5L8.5 5H12.5L9.2 7.3L10.5 11.5L7 9.1L3.5 11.5L4.8 7.3L1.5 5H5.5L7 1.5Z"
                                        fill={i < Math.round(parseFloat(avgNote)) ? '#00854B' : '#E0E0DC'} />
                                </svg>
                            ))}
                        </div>
